...
void register_key(matrix_row_t *current_row, uint8_t row, uint8_t col) {
    *current_row |= (1 << col);
    #ifdef VELOCITY_ENABLE
        register_key_velocity(row, col);
    #endif
}

void deregister_key(matrix_row_t *current_row, uint8_t row, uint8_t col) {
    *current_row &= ~(1 << col);
    #ifdef VELOCITY_ENABLE
        deregister_key_velocity(row, col);
    #endif
}

void matrix_read_cols_static_actuation(matrix_row_t *current_row, uint8_t row, uint8_t col, analog_key_t *key) {
    static uint8_t lshift_activation = 0;
    if (*current_row & (1 << col)) {
        if (key->value <= MAX(g_config.actuation_point - g_config.release_hysteresis, 0)) {
            deregister_key(current_row, row, col);
        } else if (col == 0 && row == 3 && key->value < lshift_activation - 5) {
            uprintf("LSHIFT deactivated with %d (activation %d)", key->value, lshift_activation);
            deregister_key(current_row, row, col);
        }
    } else {
        if (key->value >= MIN(g_config.actuation_point + g_config.press_hysteresis, CALIBRATION_RANGE)) {
            register_key(current_row, row, col);

            if (col == 0 && row == 3)  {
                lshift_activation = key->value;
                //uprintf("LSHIFT activated with %d (Cal_range %d)", key->value, CALIBRATION_RANGE);
            }
        }
    }
}
...
void matrix_read_cols_dynamic_actuation(matrix_row_t *current_row, uint8_t row, uint8_t col, analog_key_t *key) {
...
void matrix_read_cols_continuous_dynamic_actuation(matrix_row_t *current_row, uint8_t row, uint8_t col, analog_key_t *key) {
